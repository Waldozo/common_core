...
/*   Updated: 2025/09/10 15:03:13 by wlarbi-a         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	perform_dda(t_ray *ray, t_map *map)
{
	...
	while (hit == 0)
	{
		// Passe au prochain carré de la carte
		if (ray->side_dist_x < ray->side_dist_y)
		{
			ray->side_dist_x += ray->delta_dist_x;
			ray->map_x += ray->step_x;
			ray->side = 0;  // Collision avec un mur vertical
		}
		else
		{
...
void	init_ray(t_ray *ray, t_player *player, double camera_x)
{
	...
	// Direction du rayon = direction du joueur + plan de caméra * position x sur l'écran
...
void	render_frame(t_game *game)
{
	...
	while (x < WINDOW_WIDTH)
	{
		...
		init_ray(&ray, &game->player, camera_x);
